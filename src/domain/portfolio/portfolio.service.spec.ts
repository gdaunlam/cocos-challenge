...
const createMockPortfolioRepository = (overrides?: Partial<PortfolioRepository>): PortfolioRepository => ({
  ...
  findAllMarketData: jest.fn().mockResolvedValue(marketData),
  findAllInstruments: jest.fn().mockResolvedValue(instruments),
  ...overrides,
});
...
describe('PortfolioService', () => {
  ...
  beforeEach(() => {
    cacheService.clear();
    mockRepository = createMockPortfolioRepository();
  });
  ...
  it('should calculate portfolio with expected values', async () => {
    const service = new PortfolioService(mockRepository);
